#!/bin/bash
#SBATCH -J mpi_sm_cav_full
#SBATCH -o sm_full_cavity.out
#SBATCH -e sm_full_cavity.err
#SBATCH --no-requeue
#SBATCH --export=NONE
#SBATCH --get-user-env
    ...
    for ranks in 1 2 4 8 16; do
        for N in 63 127 255 511 1023 2047 4095 8191 16383 32767; do
            if [ $N -ge 16383 ] && [ $ranks -lt 4 ]; then
                echo "--- skipping $ranks ranks, n=$N, cavity, $sm (memory/time) ---"
                continue
            fi
            if [ $N -ge 32767 ] && [ $ranks -lt 4 ]; then
                continue
            fi
            echo "--- $ranks ranks, n=$N, cavity, $sm ---"
